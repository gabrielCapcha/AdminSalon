    ...
    $(document).ready(function() {
        ...
        //functions
            ...
            function finishNewSale() {
                ...
                if (services___ > 0) {
                    var valueAmountTaxes = valueAmount___ - parseFloat(opExoneradas) - parseFloat(opInafectas) - parseFloat(opGratuitas);
                    var subTotal___    = valueAmountTaxes * 0.7633333;
                    var taxes___       = subTotal___ * 0.18;
                } else {
                    var valueAmountTaxes = valueAmount___ - parseFloat(opExoneradas) - parseFloat(opInafectas) - parseFloat(opGratuitas);
                    var subTotal___    = valueAmountTaxes/1.18;
            ...
            function printSalePdf() {
                if (idNewSale != 0 && idNewSale != undefined) {
                    ...
                    console.log("Hubo un error con su conexión. Diríjase al listado de ventas para imprimir el ticket");
                }
            }
                ...
                if (s3Pdf != null) {
                    ...
                    if (feRuc != null) {
                        if (ticket == null) {
                            ...
                            ticket = serie + '-' + ("000000" + number).slice(-8);
                        }
                        window.open('https://sm-soft.tumi-soft.com/web/fe-document-pdf/' + feRuc + '/' + ticket);                        
                    }
                    else {
                        console.log("No se puede imprimir la venta en formato A4");
                    }
                }
            }
            function salesList() {
            ...
            function callProductsListeners(product){
                ...
                document.getElementById('quantityProduct_' + product.id).addEventListener("keyup", function(event) {
                    ...
                    if (data.selectedProducts[product.id].stock < document.getElementById('quantityProduct_' + product.id).value && product.type != 2) {
            ...
            function paymentReadjustment(paymentValue, paymentId) {
                ...
                var exchange = paymentAmount - (parseFloat(amountValue_) - generalDiscount_ - promoDiscount__);
                if (isNaN(paymentAmount) || paymentAmount < (parseFloat(amountValue_) - generalDiscount_ - promoDiscount__)) {
                    ...
                    saleErrorMessage.innerHTML = 'VALIDE SU OPERACIÓN';
                    buttonVd_ = true;
                    saleErrorMessage.className = 'btn btn-warning pull-center';
                }
                if (exchange > 0) {
        ...
        //var functions
            ...
            showProductDetail = function (id) {
                ...
                if (product.minPrice == null) {
                    ...
                    document.getElementById('productMinPrice').innerHTML = getCurrencySymbolCode(product.currency) + product.minPrice;
                }
                if (product.maxPrice == null) {
                    document.getElementById('productMaxPrice').innerHTML = 'SIN DEFINIR';
                } else {
                    document.getElementById('productMaxPrice').innerHTML = getCurrencySymbolCode(product.currency) + product.maxPrice;
                }
                            ...
                            if (count_ < 6) {
                                ...
                                if ((response[i].type == 1 && response[i].stock < 1) || response[i].flag_operation == 1) {
                                    element = element + 
                                        ...
                                        '       <h5 style="font-size:12px; font-weight:bold; margin-bottom:5px;">' + getCurrencySymbolCode((response[i].currency != null) ? response[i].currency : data.currency) + ' ' + response[i].price + '</h5>' +
            ...
            loadChangeRate = function() {
                document.getElementById('faRefresh').className = 'fa fa-refresh fa-spin';
                if (companyLoginData.exchangeRate == null) {    
                    $.ajax({
                        url: "/api/sunat-type-change",
                        context: document.body,
                        statusCode: {
                            500: function() {
                                document.getElementById('faRefresh').className = 'fa fa-refresh';
                            }
                        }
                    }).done(function(response) {
                        document.getElementById('faRefresh').className = 'fa fa-refresh';
                        // if (response.rate_compra != null) {
                        //     document.getElementById('generalCurrencyConvertionRc').value = response.rate_compra;
                        // }
                        if (response.rate_venta != null) {
                            document.getElementById('generalCurrencyConvertionRv').value = response.rate_venta;
                        } else {
                            if (response.holiday_venta != null) {
                                document.getElementById('generalCurrencyConvertionRv').value = response.holiday_venta;
                            }
                        }
                    });
                } else {
                    document.getElementById('faRefresh').className = 'fa fa-refresh';
                    document.getElementById('generalCurrencyConvertionRv').value = companyLoginData.exchangeRate;
                }
            }
            changeRate = function () {
                var generalCurrency = document.getElementById('generalCurrency');
                var generalCurrencyConvertionRv = document.getElementById('generalCurrencyConvertionRv');
                var amountValue__ = 0;
                if (generalCurrency != null && generalCurrencyConvertionRv != null && parseFloat(generalCurrencyConvertionRv.value) > 1) {
                    generalCurrency = generalCurrency.value;
                    data.currency = generalCurrency;
                    data.symbolCode = getCurrencySymbolCode(generalCurrency);
                    for (var elementValueCurrency in amountValueCurrency) {
                        var amountValueCurrency__ = 0;
                        if (elementValueCurrency == generalCurrency) {
                            amountValueCurrency__ = amountValueCurrency[elementValueCurrency].amount;
                        } else {
                            if (generalCurrency == 'USD') {
                                amountValueCurrency__ = parseFloat(amountValueCurrency[elementValueCurrency].amount / generalCurrencyConvertionRv.value);
                            } else {
                                amountValueCurrency__ = parseFloat(amountValueCurrency[elementValueCurrency].amount * generalCurrencyConvertionRv.value);
                            }
                        }
                        document.getElementById('generalCurrency_' + elementValueCurrency).value = amountValueCurrency__;
                        amountValue__ = parseFloat(amountValue__) + parseFloat(amountValueCurrency__);
                    }
                    // PAYMENTS MANAGEMENT
                    amountValue = amountValue__;
                    amountValue = amountValue - generalDiscount - promoDiscount__;
                    amountValue_ = amountValue + parseFloat(opIcbper);
                    paymentsManagement(amountValue__);
                    ...
                    if (typeDocumentCode != 'NVT') {
                        ...
                        document.getElementById('finalStepPrevSubtotal').innerHTML = '<b>OP.GRAVADAS: ' + data.symbolCode + ' ' + (parseFloat(partialAmount)/1.18).toFixed(2) + '</b>';
                        document.getElementById('finalStepPrevIgv').innerHTML = '<b>IGV: ' + data.symbolCode + ' ' + (parseFloat(partialAmount) - (parseFloat(partialAmount)/1.18)).toFixed(2) + '</b>';
                    }
                    ...
                    document.getElementById('saleErrorMessage').innerHTML = 'VALIDE SU OPERACIÓN';
                    document.getElementById('saleErrorMessage').className = 'btn btn-warning pull-center';
                }
            }
            editDetailCommentary = function (productId) {
                ...
                if (commentValidation) {
                    ...
                    alert("IMPOSIBLE DE EDITAR. COMUNICARSE CON SOPORTE.");
                }
            }
            getPartialAmount = function() {
                return parseFloat(amountValue) - parseFloat(opExoneradas) - parseFloat(opInafectas) - parseFloat(opGratuitas) - parseFloat(opIcbper);
            }
                ...
                if (data.selectedProducts.length == 0) {
                    ...
                    data.selectedProducts.forEach(element => {
                        ...
                        if (element.taxExemptionReasonCode == 20) {
                            ...
                            opIcbper = opIcbper + parseFloat(element.quantity*0.1);
                    ...
                    if (serialValidation) {
                        ...
                        if (totalProductsCount == 0) {
                            ...
                            var tBodyTableCurrencySummary = document.getElementById('tBodyTableCurrencySummary');
                            tBodyTableCurrencySummary.innerHTML = '';
                            if (tBodyTableCurrencySummary != null) {
                                var amountValueCurrencyCount = 0;
                                var amountValueCurrencyCurrency = 'PEN';
                                for (var elementValueCurrency in amountValueCurrency) {
                                    var tr = document.createElement('tr');
                                    var trTBodyTableCurrencySummary = document.createElement('tr');
                                    trTBodyTableCurrencySummary.setAttribute("style", "font-size:15px;");
                                    var trTBodyTableCurrencySummaryText_ = '<td style="vertical-align: middle;">' + amountValueCurrency[elementValueCurrency].currency + ' - ' + getCurrencySymbolCode(amountValueCurrency[elementValueCurrency].currency) + '</td>' +
                                        '<td style="vertical-align: middle;">' + parseFloat(amountValueCurrency[elementValueCurrency].amount) + '</td>' + 
                                        '<td><input class="form-control" readonly type="number" value="0.00" style="width:100%;" step="0.1" id="generalCurrency_' + amountValueCurrency[elementValueCurrency].currency + '" /></td>';
                                    trTBodyTableCurrencySummary.innerHTML = trTBodyTableCurrencySummaryText_;
                                    tBodyTableCurrencySummary.insertBefore(trTBodyTableCurrencySummary, tBodyTableCurrencySummary.firstChild);
                                    amountValue = amountValueCurrency[elementValueCurrency].amount;
                                    amountValueCurrencyCurrency = elementValueCurrency;
                                    amountValueCurrencyCount++;
                                }
                                if (amountValueCurrencyCount == 1) {
                                    data.currency = amountValueCurrencyCurrency;
                                    data.symbolCode = getCurrencySymbolCode(amountValueCurrencyCurrency);
                                    amountValue = amountValue - generalDiscount - promoDiscount__;
                                    amountValue_ = amountValue + parseFloat(opIcbper);
                                    // PAYMENTS MANAGEMENT
            ...
            cashPaymentManagement = function() {
                ...
                document.getElementById('cashManagementExchange').innerHTML = 'S/ 0.00';
                document.getElementById('cashManagementTotal').innerHTML = 'S/ 0.00';
                document.getElementById('tdMultCurTotal_PEN').innerHTML = 'S/ 0.00';
                document.getElementById('tdMultCurTotal_USD').innerHTML = 'S/ 0.00';
                ...
                if (btnSaveCashPaymentManagement != null) {
                    ...
                    switch (data.currency) {
                        case 'PEN':
                            ...
                            cashManagementExchange.innerHTML = 'S/ ' + parseFloat(cashManagementExchangeValue);
                            // TOTAL
                            document.getElementById('tdMultCurTotal_USD').innerHTML = 'S/ ' + (parseFloat(document.getElementById('cashManagementInputUSD').value)*parseFloat(document.getElementById('generalCurrencyConvertionRv').value));
                            document.getElementById('tdMultCurTotal_PEN').innerHTML = 'S/ ' + parseFloat(document.getElementById('cashManagementInputPEN').value);
                            break;
                        case 'USD':
                    ...
                    cashManagementTotal.innerHTML = 'S/ ' + cashManagementTotal_;
                    if (cashManagementExchangeValue <= 0) {
                        cashManagementExchangeValue = cashManagementExchangeValue*-1;
                        cashManagementExchange.innerHTML = 'S/ ' + cashManagementExchangeValue;
                        cashManagement = true;
                        ...
                        cashManagementExchange.innerHTML = 'S/ 0.00';
            ...
            addSelectedProduct = function (product, quantityLegal = false) {
                ...
                if (!quantityLegal) {
                    ...
                    var trinnerHTML_ = '<td class="static-table-td" onClick="showProductDetail('+ product.id +')"><strong style="cursor:pointer;">' + product.code + '</strong></td>' +
                        ...
                        '<td class="static-table-td-input"><input type="number" id="priceProduct_' + product.id + '" value="' + product.price + '" onClick="this.select();"></td>' + 
                        '<td class="static-table-td-input" style="padding-left: 0px;"><i style="color: #ffffff;" id="partialDiscountSymbol_' + product.id + '">' + getCurrencySymbolCode((product.currency != null) ? product.currency : data.currency) + ' </i><input type="number" onClick="this.select();" id="partialDiscount_' + product.id + '" value=0></td>' +
            ...
            newUserForm = function () {
                console.log("open modal-new-user");
                $('#modal-new-client-simple').modal({ backdrop: 'static', keyboard: false });
                console.log("open modal-new-user 2");
                ...
                form.innerHTML ='<div class="box-body col-md-6">' +
                    ...
                    '<select class="form-control" id="feCustomerTypeDocument">' +
                        ...
                        '<option selected value="00">SIN DOCUMENTO</option>' +
                    '</select></div>' +
                    ...
                    if (allotments.length < allotmentSelected.length) {
                        ...
                        if (typeGeneralDiscount != null) {
                            ...
                            if (generalDiscountInput != null) {
                                setInterval();
                                var amountValue___ = parseFloat(amountValue_);
                                paymentAmount = 0;
                                paymentAmounts.forEach(element => {
                                    paymentAmount = paymentAmount + parseFloat(element.value);
                                });
                                switch (typeGeneralDiscount.value) {
                                    case "0":
                                        amountValue___ = amountValue___ - parseFloat(generalDiscountInput.value);
                                        generalDiscount = parseFloat(generalDiscountInput.value);
                                        break;
                                    case "1":
                                        amountValue___ = amountValue___ - (amountValue___*parseFloat(generalDiscountInput.value)/100);
                                        generalDiscount = amountValue___*parseFloat(generalDiscountInput.value)/100;
                                        break;
                                    default:
                                        break;
                                }
                                document.getElementById('labelTotalAmount').innerHTML = 'TOTAL A PAGAR: ' + data.symbolCode + ' ' + (amountValue___).toFixed(2);
                                if (typeDocumentCode != 'NVT') {
                                    var partialAmount = getPartialAmount();
                                    document.getElementById('finalStepPrevExoneradas').innerHTML = '<b>OP.EXONERADAS: ' + data.symbolCode + ' ' + (parseFloat(opExoneradas)).toFixed(2) + '</b>';
                                    document.getElementById('finalStepPrevInafectas').innerHTML = '<b>OP.INAFECTAS: ' + data.symbolCode + ' ' + (parseFloat(opInafectas)).toFixed(2) + '</b>';
                                ...
                                // HERE finalStepPrevTotal
                                document.getElementById('finalStepPrevTotal').innerHTML = '<b>TOTAL: ' + data.symbolCode + ' ' + (amountValue___).toFixed(2) + '</b>';
                                exchangeAmount = paymentAmount - amountValue___;
                                if (documentClientValidation) {
                                    if (exchangeAmount >= 0 && (paymentAmount > 0 || amountValue___ == 0)) {
                                        document.getElementById('cashInputExchange').value = (exchangeAmount);
                                        document.getElementById('generalDiscount').readOnly = true;
                        ...
                        $.ajax({
                            ...
                            if (response.length == 0) {
                                ...
                                for (i = 0; i < response.length; i++) {
                                    ...
                                    b.innerHTML += nameLastname;
                                    b.innerHTML += "<input type='hidden' value='" + i + "'>";
                                    if ((response[i].stock > 0 || response[i].type == 2) && response[i].flag_operation != 1) {
                                        b.style.background = '#ffffff';
                                        b.style.cursor = 'pointer';
                                        b.addEventListener("click", function(e) {
                                            var iterator = this.getElementsByTagName("input")[0].value;
