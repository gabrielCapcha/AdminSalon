    ...
    $(document).ready(function() {
                    ...
                    elementObj.items.forEach(element => {
                        ...
                        if (checkBoxSearchQuotation_ != null && checkBoxSearchQuotation_.checked) {
                            ...
                            if (Number(quantitySearchQuotationValue) > 0) {
                                ...
                                console.log(selectedProductQuotation);
            ...
            if (data.customerId != genericCustomer_.id && data.customerId != 0) {
                ...
                table = $('#searchQuotationTable').DataTable({
                    ...
                    "ajax": function(data, callback, settings) {
                            $.get('/api/search-document-by-customer-id/GRM/' + customerId__, {
                                limit: data.length,
                                offset: data.start,
                                }, function(res) {
                                    var searchQuotationDataDetailResponse = '<h3>Guías disponibles: ' + res.length + '</h3>';
        ...
        secondStepOfSale = function () {
            ...
            if (data.selectedProducts.length == 0) {
                ...
                data.selectedProducts.forEach(element => {
                    var _price = document.getElementById('priceProduct_' + element.id);
                    if (_price != null) {
                        element._price = parseFloat(_price.value);
                        if (element._price < 0) {
                            element._price = 0;
                        }
                    } else {
                    ...
                    if (element.quantity > 0) {
                        ...
                        tr.innerHTML ='<td>' + element.code + '</td><td>' + element.name + '</td>' +
                            '<td>' + element._price + '</td><td>' + element.quantity + '</td>';
                        xTable.appendChild(tr); 
                        amountValue = amountValue + (element._price*element.quantity);
                    }
                    if (element.taxExemptionReasonCode == 20) {
                        opExoneradas = opExoneradas + parseFloat(element.quantity*element._price);
                    } else if (element.taxExemptionReasonCode == 30) {
                        opInafectas = opInafectas + parseFloat(element.quantity*element._price);
                    } else if (element.taxExemptionReasonCode == 21) {
                        opGratuitas = opGratuitas + parseFloat(element.quantity*element._price);
                    }
                });
                ...
                if (typeDocumentCode != 'NVT') {
                    var partialAmount = parseFloat(amountValue_) - parseFloat(opExoneradas) - parseFloat(opInafectas) - parseFloat(opGratuitas);
        ...
        addSelectedProduct = function (product, qEditable = true) {
            ...
            if (qEditable) {
                ...
                tr.innerHTML ='<td class="static-table-td" onClick="showProductDetail('+ product.id +')"><strong style="cursor:pointer;">' + product.code + '</strong></td>' +
                    ...
                    '<td class="static-table-td-input" style="padding-left: 0px;"><i style="color: #ffffff;" id="partialDiscountSymbol_' + product.id + '">' + data.symbolCode + ' </i><input type="number" step="0.1" onClick="this.select();" id="partialDiscount_' + product.id + '" value=0></td>' +
