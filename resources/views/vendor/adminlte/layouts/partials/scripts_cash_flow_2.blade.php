...
<script>
    ...
    $(document).ready(function() {
        var saleIndexTable = $('#open_cash_2').DataTable({
            "scrollX": true,
            "processing": true,
            "lengthChange": false,
            "language": {
        ...
        saveEditDocumentSubmit = function() {
            if (editDocumentId != 0) {
                ...
                $.ajax({
                    url: "/api/outcomes/" + editDocumentId,
                    ...
                    statusCode: {
                        400 : function() {
                            editDocumentId = 0;
                            alert("El retiro no se pudo editar.");
                        }
                    }
                ...
                console.log("No se pudo editar el retiro con id " + editDocumentId);
            }
        }

        documentDetailModal = function(id) {
            var detailOpenCashCreatedAt = document.getElementById('detailOpenCashCreatedAt');
            detailOpenCashCreatedAt.innerHTML = 'RETIRO DE DINERO: '  + arraySales[id].created_at;
            //ITEMS TABLE
        ...
        deleteDocument = function(id) {
            ...
            deleteSaleText.innerHTML = "¿Desea eliminar el retiro de dinero?";
        }
        ...
        deleteDocumentSubmit = function(){
            if (deletedDocumentId != 0) {
                ...
                $.ajax({
                    url: "/api/outcomes/" + deletedDocumentId + "?comment=" + comment,
                    context: document.body,
                    method: "DELETE",
                    statusCode: {
                        400:function() {
                            deletedDocumentId = 0;
                            alert("El retiro no se pudo eliminar.");
                        }
                    }
                }).done(function(response){
                    deletedDocumentId = 0;
                ...
                console.log("No se pudo eliminar el retiro con id " +deletedDocumentId);
            }
        }
        editDocument = function(id) {
            editDocumentId = id;
            var editOpenCashCreatedAt = document.getElementById('editOpenCashCreatedAt');
            editOpenCashCreatedAt.innerHTML = 'RETIRO DE DINERO: '  + arraySales[id].created_at;
            //ITEMS TABLE
