    ...
    $(document).ready(function() {
        ...
        function saveNewClient() {
            ...
            if (data.newCustomer != undefined) {
                ...
                $.ajax({
                    ...
                    if (response.flag_type_person == 1) {
        ...
        function callProductsListeners(product){
            ...
            document.getElementById('quantityProduct_' + product.id).addEventListener("keyup", function(event) {
                ...
                // } else 
                if (isNaN(quantityProduct__) || quantityProduct__ == 0 || quantityProduct__ < 0) {
            ...
            if (data.selectedProducts.length == 0) {
                ...
                data.selectedProducts.forEach(element => {
                    if (element.quantity > 0) {
                        ...
                        tr.innerHTML ='<td>' + element.code + '</td><td>' + element.name + '</td>' +
                            '<td>' + element.price + '</td><td>' + element.quantity + '</td>' + '<td>' + (parseFloat(element.quantity)*parseFloat(element.price)) + '</td>';
                ...
                if (data.customer.flag_type_person != undefined) {    
                    if (data.customer.flag_type_person == 1) {
                        pResumeText_ = '<b>' + 'Convertir cotización a: <b>' + typeDocumentName + ': </b> ' + data.symbolCode + ' ' + parseFloat(amountValue_) + '</b><br>' + data.customer.name + ', ' + data.customer.lastname + ' - DNI: ' + data.customer.dni;   
                    } else {
                        pResumeText_ = '<b>' + 'Convertir cotización a: <b>' + typeDocumentName + ': </b> ' + data.symbolCode + ' ' + parseFloat(amountValue_) + '</b><br>' + data.customer.rz_social + ' - RUC: ' + data.customer.ruc;
                    }   
                } else {
                    pResumeText_ = '<b>' + 'Convertir cotización a: <b>' + typeDocumentName + ': </b> ' + data.symbolCode + ' ' + parseFloat(amountValue_) + '</b><br>' + data.customer.name + ', ' + data.customer.lastname + ' - RUC: ' + data.customer.ruc + ' - DNI: ' + data.customer.dni;
                }
        ...
        addSelectedProduct = function (product) {
            ...
            if (quantityProduct__ != null) {
                ...
                tr.innerHTML ='<td class="static-table-td" onClick="showProductDetail('+ product.id +')"><strong style="cursor:pointer;">' + product.code + '</strong></td>' +
                    ...
                    '<td class="static-table-td-input" style="padding-left: 0px;"><i style="color: #ffffff;" id="partialDiscountSymbol_' + product.id + '">' + data.symbolCode + ' </i><input type="number" onClick="this.select();" id="partialDiscount_' + product.id + '" value=0></td>' +
        ...
        generalDiscount_.addEventListener("keyup", function(event) {
            ...
            var pResume = document.getElementById('totalResumeAmount');
            pResume.innerHTML = 'Convertir cotización a: <b>' + typeDocumentName + ': </b> ' + data.symbolCode + ' ' + parseFloat(amountValue);
            ...
            if (typeDocumentCode != 'NVT') {
                // HERE finalStepPrevSubtotal
                document.getElementById('finalStepPrevSubtotal').innerHTML = 'SUBTOTAL: ' + data.symbolCode + ' ' + (parseFloat(amountValue)/1.18);
                // HERE finalStepPrevIgv
                document.getElementById('finalStepPrevIgv').innerHTML = 'IGV: ' + data.symbolCode + ' ' + (parseFloat(amountValue) - (parseFloat(amountValue)/1.18));
            }
            // HERE finalStepPrevTotal
            document.getElementById('finalStepPrevTotal').innerHTML = 'TOTAL: ' + data.symbolCode + ' ' + amountValue;
            setInterval();
        });
